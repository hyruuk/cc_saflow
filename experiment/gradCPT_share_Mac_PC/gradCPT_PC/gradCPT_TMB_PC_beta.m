...
def={'XXX','.05','NA','0','0','0','.9','.05','0','2','16'};
...
jpeg1 = imread([picDir_SF 'SF1' num2str(randi(10)) '.jpg']);
[x,y]=size(jpeg1);
%to resize jpeg1%%%%
...
%start with noise
jpeg2 = imread([picDir_SR 'SS1' '.jpg']); %start with random scene
...
%     CenterText2(window,['actual duration: ' num2str(duration)],[0 0 0],0, 200);
    ...
    scene_type_r2=trial_list(n,3); %use trial_list
    
    if scene_type_r2~=0
        
        scene_r2=trial_list(n,4);
...
%         if fMRI==0 %& j<25
            ...
            if data(n,10)==55        %Bail out if they press '7'
                %                 clear mex;
                %                 stopit;
                clear mex;
                stopitnow;
...
%         [Results]= thought_probe_func2(window);
