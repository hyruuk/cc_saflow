...
CE_Slope=CE_S(1)/(avg_trial_length*quartile/60);

OE_S=polyfit(1:1:4,omission_rate(2:5),1);
OE_Slope=OE_S(1)/(avg_trial_length*quartile/60);

RT_S=polyfit(1:1:4,meanRT(2:5),1);
RT_Slope=RT_S(1)/(avg_trial_length*quartile/60);

STD_S=polyfit(1:1:4,STD_RT(2:5),1);
STD_Slope=STD_S(1)/(avg_trial_length*quartile/60);

Err_S=polyfit(1:1:4,error_rate(2:5),1);
Err_Slope=Err_S(1)/(avg_trial_length*quartile/60);

dprime_S=polyfit(1:1:4,dprime(2:5),1);
dprime_Slope=dprime_S(1)/(avg_trial_length*quartile/60);

criterion_S=polyfit(1:1:4,criterion(2:5),1);
criterion_Slope=criterion_S(1)/(avg_trial_length*quartile/60);

CV_S=polyfit(1:1:4,CV(2:5),1);
CV_Slope=CV_S(1)/(avg_trial_length*quartile/60);
