...
%YH :
%To change :
% - remove SkipSyncTests - check
% - add parallel port setup -check
% - add triggers for TrialStart (first appearance of the new image) - check
% - add triggers for Response - check
% - change default parameters - check
% - change response key left (4) and right (7) -ok ?
...
r1=52; % ' ' for response key 1 (cat2)     male/city

r2=55;  % '.' for response key 2 (cat1)     female/mountain
...
prompt = {'Enter subject name','Block','Rate','Duration','fMRI','Prac','FHR','Prob','Hold','Scram','Task','framesper'};
def={'XXX','0','.05','480','0','0','0','.9','.05','0','2','16'};
answer = inputdlg(prompt, 'Experimental setup information',1,def);
[subName, block, Rate, duration, fMRI, Prac, FHR, Prob, Hold,SR,Task,framesper] = deal(answer{:});
% Rate = time between consecutive frames
% framsper = number of frames per image (blended with i-1 and i+1 -> how ?)
% duration = total bloc duration
...
Screen('Preference', 'SkipSyncTests', 1);  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% skip sync test here
Screen('TextFont',window, 'Geneva');
...
for n=1:numberoftrials
    ...
    if list.pics(n,1) == 0
        %outp(address, TriggBlocStart);
        WaitSecs(0.0009765625);
        %outp(address,0);
    end

    if list.pics(n,1) == 2
        %outp(address, TriggFreqStart);
        WaitSecs(0.0009765625); %% This pause is to make sure that the trigger signal stayed long enough for the amplifier to catch it.
        %outp(address,0);        %% please note that the actual stimulus onset is ~1 ms later than the trigger
    end

    if list.pics(n,1) == 1
        %outp(address, TriggRareStart);
        WaitSecs(0.0009765625); %% This pause is to make sure that the trigger signal stayed long enough for the amplifier to catch it.
        %outp(address,0);        %% please note that the actual stimulus onset is ~1 ms later than the trigger
    end
...
%         if fMRI==0 %& j<25
            while GetSecs-StimulusOnsetTime<(WaitTime-.030),
                ...
                if keyIsDown==1 & res_trial==0 & (j-last_resp_frame>5 | last_resp_frame==0);
                    ...
                    % 7 = 55
                    % 4 = 52
                    key_pressed = sum(find(keyCode))
                    disp(key_pressed);
                    if sum(find(keyCode))== 52;
                        %outp(address, TriggResp);
                        sprintf('4');
                        WaitSecs(0.0009765625);
                        %outp(address,0);
            ...
            if data(n,10)==27        %Bail out if they press 'escape'
                %                 clear mex;
                %                 stopit;
                clear mex;
                stopitnow;
...
CenterText2(window,['Fermez les yeux'],[0 0 0],0, 300);
...
cd data
'saving data- wait!'
save(['Data_' num2str(Prac) '_' subName '_' block '_' date '_' num2str(C(4)) '_' num2str(C(5)) '_city_mnt_v1B_beh_fMRI.mat'],'data', 'response','ttt','FHR','Rate','Prob','Task','Scram', 'framesper', 'numberoftrials', 'subName','starttime','endtime','x_size','y_size');

%%% ajouter n_bloc au nom du fichier

cd ..
WaitSecs(1);

%%%% � la fin ajouter un screen avec "Fermez les yeux jusqu'aux prochaines
%%%% instructions"
