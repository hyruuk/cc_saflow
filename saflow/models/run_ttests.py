...
if __name__ == "__main__":
    ...
    for FREQ in range(len(FREQS_NAMES)):
        ...
        masks.append(create_pval_mask(results["pvals"], from saflow import (
    BIDS_PATH,
    SUBJ_LIST,
    BLOCS_LIST,
    FREQS_NAMES,
...
if __name__ == "__main__":
    ...
    if by == "VTC":
        ...
        figpath_contrast = (
            IMG_DIR
            + "PSD_contrast_{}perm_alpha{}_{}{}_{}.png".format(
                n_perms, str(alpha)[2:], split[0], split[1], correction
            )
        )
    elif by == "odd":
        conds_list = ["FREQhits", "RAREhits"]
        balance = True
        savepath = RESULTS_PATH + "{}_PSD_ttest_{}perm_{}/".format(by, n_perms, correction)
